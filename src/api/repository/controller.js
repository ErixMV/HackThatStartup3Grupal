...
const getOne = async (req, res) => {
    const { id } = req.params,
        [err, repoFound] = await to(rp.find({ _id: id }));

    if (err)
        return send(res, 404, err.message);

    return send(res, 200, repoFound);
}

const add = async (req, res) => {
    const { body } = req,
        [err, newRepo] = await to(rp.addOne(body));

    if (err)
        return send(res, 400, err.message);

    return send(res, 201, newRepo);
}
...
const deleteOne = async (req, res) => {
    ...
    if (err || !deletedDoc)
        return send(res, 404, err ? err.message : 'Repository not found');
